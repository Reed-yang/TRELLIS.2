...
Write a SINGLE markdown file: `tmp/pretriton_s4_analysis.md`

The file MUST follow this exact structure (sections A through E). Total length < 300 lines.

```
# S4 Pre-Triton Analysis

## A. 当前实现快照（针对 corep_fast/stages/s4_face_point.py 当前 HEAD）
- 关键函数 + 行号
...
- `_compute_face_weights_mp` (line 132) + `_fw_worker_indexed` (line 171) — old MP path (may be dead code now)
- `_compute_face_weights_gpu` (line 1119) — M2 P2 GPU path
- M2 spec proposed `_uturn_worker` (verify actual name in current code by grep)
...
Be **very thorough**: read every line of `_compute_face_weights_gpu` and trace exactly where data enters/leaves GPU vs CPU. Cite line numbers for every claim.
