...
Tolerance policy:
- V / F scalar counts: exact match within-corep_fast (we're comparing the
  same code base before/after internal rewrites, FP nondeterminism should
  not flip V/F counts). A single mismatch is a test failure.
- Final mesh tensors (V×3 vertices, F×3 faces): max abs diff <= 1e-5
  (vertex positions) AND exact integer match (face indices).
- Internal intermediate tensors (cube_indices, edge_weights, etc.) if
  surfaced: exact match for integers, max_abs_diff <= 1e-6 for floats.
...
Generate goldens once:
    GENERATE_GOLDEN=1 pytest corep_fast/tests/regression/test_cpu_worker_optim.py \\
        -v --run-generate
...
def _triple_icosphere() -> trimesh.Trimesh:
    """Three concentric icospheres: r=1.00, 1.01, 1.02, subdiv=3 each.
...
def _run_pipeline(mesh_factory, resolution: int):
    ...
    batch, v, f = corep_pipeline(mesh, resolution, device)
...
def get_pool(num_workers: int) -> _mp.pool.Pool:
    ...
    if _pool is not None:
        shutdown_pool()
    _pool = _mp.Pool(processes=num_workers)
